import { Connection, SrooshClient, version } from "../";
...
const DEFAULT_DC_ID = 3;
const DEFAULT_IPV4_IP = isNode ? "im-server.splus.ir" : "im-server.splus.ir";
 ...
 */
export interface SrooshClientParams {
...
const clientParamsDefault = {
    connection: isNode ? ConnectionWebSocket : ConnectionTCPObfuscated,
...
export abstract class TelegramBaseClient {
    ...
    constructor(
        ...
        apiId: number,
        apiHash: string,
        clientParams: SrooshClientParams
    ) {
        ...
        if (this.useWSS && this._proxy) {
            throw new Error(
                "Cannot use SSL with proxies. You need to disable the useWSS client param in SrooshClient"
            );
        }
    ...
    _createExportedSender(dcId: number) {
        return new MTProtoSender(this.session.getAuthKey(dcId), {
            ...
            client: this as unknown as SrooshClient,
